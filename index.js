...
setTitle("Justicee Token Generator");
const fs = require("fs");
async function main() {
  console.clear();
  ...
  console.log("Justicee#2010 tarafından kodlanmıştır");
  console.log("[" + "1".green + "] [" + "Başlat".green + "]");
  ...
  if (choice == 1) {
    ...
    console.log(
      "[" +
        "!".red +
        "] Tokenleri WebHook a göndermek istiyormusun (Y=evet,N=Hayır) y/n: "
    );
    ...
    if (choicewbwile2 == "y") {
      sunucudavet = prompt("Sunucu Davet Bağlantısı: ");
    }
    let tokensname = prompt(
      "Tokens username: ( Boş Bırakırsanız Kendisi Random Yapıyor ) "
    );
    let HowTokens = prompt("Kaç Token Oluşturmak İstiyorsun: ");
    ...
    async function create_accinfos(chrome, disc) {
      ...
      const password = "Muhammed20075345"; //TÜM OLUSTURDUGUN TOKENLERİN ŞİFRELERİ BU OLACAK
    ...
    console.log("Yardım almak için Justicee#2010 Ekleyerek Yardım Alabilirsiniz");
